module.exports = {
    http_createServer_callback: function(request, response) {
        var nodehttpserverrequest = request;
        var nodehttpserverresponse = response;
        response.disable_auto_response = true; 
        ...
        var get_mimeend_by_filenameorpath = function(filenameorpath){
            var mimeend = filenameorpath.split(".").pop();
            mimeend = mimeend.replace("jpg", "jpeg");
            return mimeend; 
        };

        var day_ts = parseInt(new Date().getTime() / 1000 / (60 * 60 * 24)) * 60 * 60 * 24;
        var hour_ts = parseInt(new Date().getTime() / 1000 / (60 * 60)) * 60 * 60;
        var interval_ts = hour_ts; 


        //check if image for current interval already exists 
        var files = fs.readdirSync("./"+images_directory).filter(
            fn => fn.startsWith('from_canvas_'+interval_ts)
        );
        if(files.length > 0){
            var filepath = "./"+images_directory+"/"+files[0];
            var mimeend = get_mimeend_by_filenameorpath(filepath);
            nodehttpserverresponse.writeHead(200, {
                "Content-Type": "image/"+mimeend,
            });
            fs.createReadStream(filepath).pipe(nodehttpserverresponse);
            
            return {response:nodehttpserverresponse, request:nodehttpserverrequest};
            console.log(files);
        ...
        var draw_text_with_bg_square = function(ctx, options) {
            ...
            var padding_xy_object = (options.padding_xy_object) ? options.padding_xy_object : {
                x: 20,
                y: 20
            };

            var location_xy_object = (options.location_xy_object) ? options.location_xy_object : {
                x: 20,
                y: 20
            };

            var alignment_xy_object = (options.alignment_xy_object) ? options.alignment_xy_object : {
                x: "end",
                y: "end"
            };
        ...
        var url =
            `https://hubblesite.org/resource-gallery/images?Topic=105-galaxies&filterUUID=4c394bbb-b21e-43ab-a160-2a4521d70243&page=${rand_page}&itemsPerPage=15&`
        var return_data = {}
        return_data.urls = [url];
        make_request({
            url: url
        }, function(document) {
            ...
            return_data.urls.push(absolute_url);
            ...
            make_request({
                ...
                https.request(absolute_url, function(response) {
                    ...
                    response.on('end', function() {
                        ...
                        var original_image_file_name = interval_ts + "_" + img_url.split("/").pop()
                        ...
                        //  fs.writeFileSync(interval_ts+"_"+img_url.split("/").pop(), buffer);       

                        loadImage("./" + images_directory + "/" + original_image_file_name).then(image => {
                            var canvas_image_path_file_name = "from_canvas_" +
                                original_image_file_name;
                                var mimeend = get_mimeend_by_filenameorpath(canvas_image_path_file_name);
                            ...
                            fs.writeFileSync("./" + images_directory + "/" +
                                canvas_image_path_file_name, buffer)
                            ...
                            fs.unlink("./" + images_directory + "/" + original_image_file_name);
                                ...
                                nodehttpserverresponse.end(buffer, 'binary');
                            // return {
                            //     msg: "success",
                            //     data: return_data
                            // }
